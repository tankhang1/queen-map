...
const R6Report = ({ filterDate }: { filterDate: TFilter }) => {
  const { data: r6Report } = useGetReportQuery(
    {
      code: "R6",
  ...
  // Group logs by date
  const statsByDate = (r6Report || []).reduce((acc, log) => {
  ...
  return (
    <Stack>
      <Text fw={"bold"}>Sầu riêng Ri 6</Text>
